...
(defn- aggregation-subclause?
  ...
  (or (when ((some-fn keyword? string?) x)
        (#{:avg :count :cum-count :distinct :percent-of-total-sum :stddev :sum :min :max :+ :- :/ :*
...
(doseq [clause-name [:avg :cum-sum :distinct :percent-of-total-sum :stddev :sum :min :max :median :var]]
  (defmethod canonicalize-mbql-clause clause-name
    [clause]
