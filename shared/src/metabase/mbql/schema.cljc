...
(def arithmetic-expressions
  ...
  #{:+ :- :/ :* :coalesce :length :round :ceil :floor :abs :power :sqrt :log :exp :ratio-to-report :case})

(def boolean-expressions
  "Set of valid boolean expression clause keywords."
  #{:and :or :not :< :<= :> :>= := :!=})

(def ^:private aggregations #{:sum :avg :percent-of-total :stddev :var :median :percentile :min :max :cum-count :cum-sum :count-where :sum-where :share :distinct :metric :aggregation-options :count})
...
(defclause ^{:requires-features #{:basic-aggregations :window-functions}} ratio-to-report
  x NumericExpressionArg)
...
(def ^:private ArithmeticExpression*
  (one-of + - / * coalesce length floor ceil round abs power sqrt exp log ratio-to-report case))
...
(defclause ^{:requires-features #{:basic-aggregations :window-functions}} ^:sugar percent-of-total
...
(def ^:private UnnamedAggregation*
  (s/if (partial is-clause? arithmetic-expressions)
    ...
    (one-of count avg cum-count cum-sum distinct percent-of-total stddev sum min max metric share count-where
